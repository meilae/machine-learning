...
fprintf('Training ... ');
tic;
[phi, mu0, mu1, Sigma] = GDATrain_WONGMEILING( DataTrain, LabelsTrain );
% [phi, mu0, mu1, Sigma] = GDATrain_SOLUTION( DataTrain, LabelsTrain );
toc;

%%
% Test Gaussian Discriminant Analysis.
% p(y=1|x) We ignore the constant of the gaussian
x1 = bsxfun(@minus, DataITest', mu1);
logp1 = log(phi) + sum( -x1.*(inv(Sigma)*x1)/2, 1 );
% p(y=0|x)
x0 = bsxfun(@minus, DataITest', mu0);
logp0 = log(1-phi) + sum( -x0.*(inv(Sigma)*x0)/2, 1 );
% The score is the log-likelyhood ratio
scores = logp1 - logp0;
